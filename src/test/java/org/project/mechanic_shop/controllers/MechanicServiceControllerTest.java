...
class MechanicServiceControllerTest {
	...
	class FindById {
		...
		void shouldFindUserByExternalId() throws Exception {
			...
			mockMvc
				.perform(get(String.format("/api/mechanic-services/%s", externalId)))
	...
	class Search {
		...
		void shouldSearchWithPageable() throws Exception {
			...
			Page<MechanicService> page = new PageImpl<>(listMechanic, PageRequest.of(0, 2), 1);

			when(service.search(eq(mechanic.getName()), any(Pageable.class))).thenReturn(page);
			...
			verify(service).search(eq(mechanic.getName()), captor.capture());
		...
		void shouldSearchWithPageableDefault() throws Exception {
			...
			mockMvc
				.perform(get("/api/mechanic-services/search").param("name", mechanic.getName()))
				.andExpect(status().isOk())
				.andExpect(jsonPath("$.status").value(HttpStatus.OK.value()))
				.andExpect(jsonPath("$.message").value("success"))
				.andExpect(jsonPath("$.data.content").isArray())
