...
class UserControllerIT {
    ...
    void shouldReturnValidationErrorWhenCreatePayloadIsInvalid() throws Exception {
        UserManDto invalidPayload = new UserManDto(
                "",
                "",
                "invalid-email",
                null,
    ...
    void shouldFindUserByExternalIdUsingRealRepository() throws Exception {
        User savedUser = userRepository.save(buildUser(
                "11144477735",
                "Integration Find",
                "integration.find@test.com",
                "11999990002"
        ));

        mockMvc.perform(
                        get("/api/users/{id}", savedUser.getExternalId())
                                .with(AuthUtil.admin())
                )
    ...
    void shouldUpdateUserUsingRealServiceAndRepository() throws Exception {
        User savedUser = userRepository.save(buildUser(
                ...
                "integration.update.before@test.com",
                "11999990003"
        ));
    ...
    void shouldSearchUsersUsingPersistedData() throws Exception {
        ...
        mockMvc.perform(
                        get("/api/users/search")
                                .with(AuthUtil.admin())
                                .param("document", savedUser.getDocument())
                )
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.status").value(HttpStatus.OK.value()))
                .andExpect(jsonPath("$.message").value("success"))
