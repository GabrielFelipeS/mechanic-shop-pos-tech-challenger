...
class StockItemControllerIT extends AbstractIntegrationTest {
	...
	void shouldReturnValidationErrorWhenCreatePayloadIsInvalid() throws Exception {
		StockItemManDto invalidPayload = new StockItemManDto("", "", null, "<b>descricao</b>", -1, null, null);
	...
	void shouldFindStockItemByExternalIdUsingRealRepository() throws Exception {
		...
		mockMvc
			.perform(get("/api/stock-items/{id}", savedItem.getExternalId()).with(AuthUtil.admin()))
	...
	void shouldSearchStockItemsUsingPersistedData() throws Exception {
		...
		mockMvc
			.perform(get("/api/stock-items/search").with(AuthUtil.admin()).param("code", savedItem.getCode()))
			.andExpect(status().isOk())
			.andExpect(jsonPath("$.status").value(HttpStatus.OK.value()))
			.andExpect(jsonPath("$.message").value("success"))
