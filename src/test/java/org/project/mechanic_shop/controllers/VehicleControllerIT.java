...
class VehicleControllerIT {
	...
	void shouldCreateVehicleAndPersistIt() throws Exception {
		User owner = userRepository.save(
			buildUser("52998224725", "Dono Integracao", "owner.vehicle.create@test.com", "11999991001")
		);
		...
		Vehicle persistedVehicle = vehicleRepository
			.findByExternalId(java.util.UUID.fromString(objectMapper.readTree(responseBody).get("data").asText()))
			.orElseThrow();
	...
	void shouldReturnValidationErrorWhenCreatePayloadIsInvalid() throws Exception {
		VehicleManDto invalidPayload = new VehicleManDto("INVALIDA", "", "", 1800, "<b>preto</b>", null);
	...
	void shouldFindVehicleByExternalIdUsingRealRepository() throws Exception {
		User owner = userRepository.save(
			buildUser("11144477735", "Dono Busca", "owner.vehicle.find@test.com", "11999991002")
		);
		Vehicle savedVehicle = vehicleRepository.save(buildVehicle("DEF1G45", "Fiat", "Argo", owner));

		mockMvc
			.perform(get("/api/vehicles/{id}", savedVehicle.getExternalId()).with(AuthUtil.admin()))
	...
	void shouldUpdateVehicleUsingRealServiceAndRepository() throws Exception {
		User originalOwner = userRepository.save(
			buildUser("16899535009", "Dono Original", "owner.vehicle.original@test.com", "11999991003")
		);
		User updatedOwner = userRepository.save(
			buildUser("39053344705", "Dono Atualizado", "owner.vehicle.updated@test.com", "11999991004")
		);
	...
	void shouldSearchVehiclesUsingPersistedData() throws Exception {
		User owner = userRepository.save(
			buildUser("45317828791", "Dono Pesquisa", "owner.vehicle.search@test.com", "11999991005")
		);
		Vehicle savedVehicle = vehicleRepository.save(buildVehicle("NOP3Q12", "Toyota", "Yaris", owner));

		mockMvc
			.perform(
				get("/api/vehicles/search").with(AuthUtil.admin()).param("licensePlate", savedVehicle.getLicensePlate())
			)
			.andExpect(status().isOk())
			.andExpect(jsonPath("$.status").value(HttpStatus.OK.value()))
			.andExpect(jsonPath("$.message").value("success"))
